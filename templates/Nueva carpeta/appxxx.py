...
def asignar_qr(cliente_id):
    ...
    if request.method == 'POST':
        ...
        for _ in range(cantidad_codigos):
            ...
            nuevo_qr = CodigoQR(codigo=nuevo_codigo, cliente_id=cliente.id, numero_correlativo=siguiente_numero)
            db.session.add(nuevo_qr)
            qr = qrcode.make(nuevo_codigo)
            qr.save(os.path.join('static/qr_codes', f"{nuevo_codigo}.png"))
            siguiente_numero += 1
        db.session.commit()
...
def reporte():
    ...
    total_recaudado = 0
    total_pendiente = 0
    PRECIO_POR_QR = 20.0  # Precio fijo
